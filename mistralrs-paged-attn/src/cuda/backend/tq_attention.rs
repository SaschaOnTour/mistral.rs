...
///
/// Two-phase: partitioned attention + reduce. Each partition is processed
/// by a separate CUDA block for maximum GPU utilization.
/// Temporary buffers are allocated as Candle tensors (freed automatically).
pub fn tq_fused_attention(
    ...
    let Device::Cuda(dev) = device else {
        candle_core::bail!("tq_fused_attention requires CUDA device");
    };
    ...
    // Temporary buffers for partitioned results (allocated once, freed after kernel)
